window.addEventListener("load", function () {
  ...
  nombre.addEventListener("keypress", (e) => {
    if (validarNombre(e.key) == false) {
      const error = mostrarMensajeEnElemento(
        "El campo nombre no se admiten números."
      );
  ...
  apellido.addEventListener("keypress", (e) => {
    if (validarNombre(e.key) == false) {
      const error = mostrarMensajeEnElemento(
        "El campo apellido no se admiten números."
      );
      e.target.parentNode.appendChild(error);
      e.preventDefault();
    }
  ...
  function validatePass(e) {
    if (validarContrasenia(e.target.value) == false) {
      const error = mostrarMensajeEnElemento(
        'La contraseña debe tener mas de 3 dígitos y no puede contener " " ni  "-".'
      );
      e.target.parentNode.appendChild(error);
      e.preventDefault();
    }
  ...
  pass1.addEventListener("blur", (e) => {
    if (
      validarMinimoContraseña(pass1.value) == false &&
      compararContrasenias(pass1.value, pass2.value) == false
    ) {
      const error = mostrarMensajeEnElemento(
        "La contraseñas debe tener un mínimo de 3 dígitos y deben coincidir."
      );
      e.target.parentNode.appendChild(error);
    }
  });
  ...
  pass2.addEventListener("blur", (e) => {
    if (
      validarMinimoContraseña(pass2.value) == false &&
      compararContrasenias(pass1.value, pass2.value) == false
    ) {
      const error = mostrarMensajeEnElemento(
        "La contraseñas debe tener un mínimo de 3 dígitos y deben coincidir."
